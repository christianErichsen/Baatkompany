// server.js
// Enkel Node/Express-app for kjøp, salg og reparasjon av båter
// Kjør lokalt: `npm install` deretter `npm start`
// Deploy til Heroku: push repo med denne fila + package.json + Procfile (se instruksene i chatten)
...
// Midlertidig "database" i minne (erstatt med Postgres senere)
let listings = [
  {
    id: 1,
    title: 'Uttern 4602 (2004)',
    priceNOK: 45000,
    location: 'Oslo',
    description: 'Velholdt skjærgårdsjeep. 40hk Mercury, nylig servet.',
    phone: '+47 900 00 000'
  },
  {
    id: 2,
    title: 'Askeladden 475 Freestyle (2011)',
    priceNOK: 125000,
    location: 'Bergen',
    description: 'Klar for sommeren. Garmin kartplotter, kalesje.',
    phone: '+47 901 23 456'
  }
];
...
app.get('/', (req, res) => {
  const cards = listings.map(l => `
    <div class="rounded-2xl border bg-white shadow-sm p-4 flex flex-col">
      ...
      <p class="mt-2 font-bold">${currency(l.priceNOK)}</p>
...
app.post('/sell', (req, res) => {
  ...
  const item = { id, title, priceNOK: Number(priceNOK), location, description, phone };
  listings.unshift(item);
  sellSubmissions.unshift({ ...item, createdAt: new Date().toISOString() });
  res.redirect('/#kjop');
});

app.post('/repair', async (req, res) => {
  const { name, phone, boat, issue } = req.body;
  const ticket = { id: repairRequests.length + 1, name, phone, boat, issue, createdAt: new Date().toISOString() };
  repairRequests.unshift(ticket);
  ...
  try {
    if (process.env.SENDGRID_API_KEY && process.env.TO_EMAIL) {
      ...
      await sg.send({
        ...
        subject: `Ny serviceforespørsel #${ticket.id} – ${name}`,
        text: `Navn: ${name}\nTelefon: ${phone}\nBåt: ${boat}\n\nProblem:\n${issue}`
      });
    }
  } catch (e) {
    console.error('Klarte ikke å sende e‑post:', e.message);
  }
...
app.get('/admin', (req, res) => {
  ...
  const items = listings.map(l => `<li><strong>${l.title}</strong> – ${currency(l.priceNOK)} – ${l.location}</li>`).join('');
  const repairs = repairRequests.map(r => `<li>#${r.id} – ${r.name} (${r.phone}) – ${r.boat}</li>`).join('');
  const sells = sellSubmissions.map(s => `<li>#${s.id} – ${s.title} – ${currency(s.priceNOK)}</li>`).join('');
  ...
  res.send(layout('Admin', `
    <div class="grid lg:grid-cols-3 gap-6">
      ...
      <div class="p-6 bg-white rounded-2xl border">
        <h3 class="font-semibold">Serviceforespørsler (${repairRequests.length})</h3>
        <ul class="mt-2 list-disc ml-5 text-sm">${repairs || '<li>Ingen</li>'}</ul>
      </div>
      <div class="p-6 bg-white rounded-2xl border">
        <h3 class="font-semibold">Innsendte salgsdata (${sellSubmissions.length})</h3>
        <ul class="mt-2 list-disc ml-5 text-sm">${sells || '<li>Ingen</li>'}</ul>
      </div>
    </div>
  `));
});
